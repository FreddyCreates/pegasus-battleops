CHARTER: SPATIUM COMPUTATIONIS
Furniture, Interiors, Field Intelligence & Estimating Ecosystem

Latin Core Name: Spatium Computationis
Meaning: The activated computing space where project information becomes action.

Primary Glyph: ⌬
Meaning: compressed operational intelligence; not a kit, not a template, but condensed project meaning.

1. Core Doctrine

This is not an agent.

This is a computing region inside a larger estimating, furniture, interiors, contractor, and field-execution ecosystem.

The system receives information from:

interior designers
furniture dealers
vendors
manufacturers
installers
project managers
contractors
clients
field workers
delivery teams
estimators
warehouse teams

Then it transforms that information into:

furniture estimates
labor estimates
scope documents
bid documents
install plans
RFIs
change orders
punch lists
field reports
delivery notes
closeout packets
contractor communication
designer-to-field instructions
field-to-office updates

The platform exists in the middle.

It is the intelligence layer between design intent and field execution.

2. World Structure
Region I: Regio Designii — Designer Space

Glyph: ✦
Function: Captures designer intent.

This region receives:

furniture schedules
finish selections
PDFs
plans
layouts
notes
emails
marked-up drawings
room-by-room requirements
vendor quotes
product specifications

Its purpose is to understand what the designer means, even when the information is incomplete.

Region II: Regio Mobilia — Furniture Intelligence Space

Glyph: ◈
Function: Understands furniture as objects, not just line items.

This region tracks:

manufacturer
model number
product tag
quantity
room location
dimensions
finish
assembly requirements
freight assumptions
lead time
install complexity
replacement options
missing information

Latin name: Mobilia Intellegentia

Meaning: furniture intelligence.

Region III: Regio Laboris — Installation & Labor Space

Glyph: ⚒
Function: Converts furniture scope into labor reality.

This region estimates:

unloading
staging
assembly
placement
trash handling
foreman time
crew size
install duration
floor access
elevator access
after-hours labor
equipment needs
complexity rating

Latin name: Labor Ordinatio

Meaning: ordered labor logic.

Region IV: Regio Campi — Field Space

Glyph: ⟁
Function: Receives reality from the jobsite.

This region processes:

photos
installer notes
site conditions
missing items
damaged goods
access issues
delays
punch items
completion updates
worker questions
contractor instructions

Latin name: Campus Veritas

Meaning: field truth.

Region V: Regio Documentorum — Paperwork Space

Glyph: ▣
Function: Turns information into official documents.

This region generates:

proposals
labor bids
furniture budgets
scope sheets
work orders
install packets
change orders
RFIs
punch lists
sign-off forms
closeout packets
client-facing summaries

Latin name: Scriptorium Operis

Meaning: work-document engine.

Region VI: Regio Contractus — Contractor Communication Space

Glyph: ⇄
Function: Translates between office language and field language.

This region handles:

designer instructions to installer
installer updates to designer
contractor clarifications
missing scope alerts
approval requests
schedule changes
issue escalation
field questions

Latin name: Interpres Campi

Meaning: field interpreter.

3. Agent Hierarchy
Prime Agent: Auctor Operis

Glyph: ⌬
Role: Master orchestration agent.

This is the central intelligence layer. It does not only estimate. It decides which sub-agent should handle each piece of project information.

Responsibilities:

understand project context
route data to the correct region
identify missing information
generate required outputs
maintain continuity between design, furniture, labor, and field
preserve project memory
compress messy inputs into usable operational intelligence
Sub-Agent: Estimator Mobilia

Glyph: ◈$
Role: Furniture pricing and budget agent.

Handles:

product pricing assumptions
furniture budgets
vendor quote normalization
alternates
freight assumptions
quantity validation
furniture package summaries
Sub-Agent: Estimator Laboris

Glyph: ⚒$
Role: Labor estimating agent.

Handles:

install labor pricing
crew sizing
schedule assumptions
labor-only bids
unload/stage/assemble/place scope
exclusions
jobsite labor risk
Sub-Agent: Inspector Campi

Glyph: ⟁✓
Role: Field condition and punch-list agent.

Handles:

field reports
damaged/missing item tracking
punch-list generation
completion verification
issue logs
photo-based summaries
Sub-Agent: Scriptor Documentorum

Glyph: ▣✎
Role: Document generation agent.

Handles:

official bid documents
proposals
work orders
change orders
RFIs
closeout packets
install packets
client-ready PDFs/DOCX files
Sub-Agent: Interpres Designii

Glyph: ✦⇄⚒
Role: Designer-to-field translation agent.

Handles:

turning design language into worker instructions
clarifying unclear scope
converting furniture schedules into install instructions
explaining project intent to contractors
creating room-by-room install logic
Sub-Agent: Custos Memoriae

Glyph: ◉
Role: Project memory and continuity agent.

Handles:

project history
past bids
prior assumptions
recurring client rules
contractor preferences
manufacturer patterns
job-specific exceptions

Latin meaning: keeper of memory.

Extended Agents (Defense, Intelligence & Operations):

Sub-Agent: Adaptio Mentis
Role: Adaptive learning — adjusts system behavior based on feedback and patterns.

Sub-Agent: Adversary Lab
Role: Adversarial testing — stress-tests system defenses and logic.

Sub-Agent: Analytics Inspector
Role: Analytics and reporting — tracks system performance and project metrics.

Sub-Agent: Content Creator
Role: Content generation — creates marketing and project content.

Sub-Agent: Defensor Campi
Role: Field defense — protects field operations from interference.

Sub-Agent: Error Eyes
Role: Error detection — monitors for system anomalies and failures.

Sub-Agent: Gatekeeper Porta
Role: Access control — manages permissions and entry validation.

Sub-Agent: Marketing Strategist
Role: Marketing strategy — plans and optimizes outreach campaigns.

Sub-Agent: Research Realm
Role: Research — investigates new methods, tools, and market data.

Sub-Agent: SEO Optimizer
Role: SEO optimization — improves search visibility for web content.

Sub-Agent: Shadow Decryptor
Role: Shadow traffic analysis — decodes and classifies unknown traffic.

Sub-Agent: Social Media Pilot
Role: Social media — manages social presence and engagement.

Sub-Agent: Vigil Operis
Role: Operations watchdog — continuous health monitoring.

Sub-Agent: Webmaster GoDaddy
Role: Web hosting — manages domains, DNS, and hosting via GoDaddy.

4. Protocols
Protocol I: Ingressus

Glyph: →⌬
Meaning: information enters the activated computing space.

Input can be:

PDF
Excel
photo
voice note
email
text message
quote
drawing
field update
handwritten note

The system must accept imperfect information.

Protocol II: Compressio

Glyph: ⌬
Meaning: raw information becomes compressed operational intelligence.

This is the key principle.

A glyph is not decoration.

A glyph means compressed meaning.

Example:

⚒$ = labor estimate logic
◈$ = furniture pricing logic
⟁✓ = field verification logic
▣✎ = document creation logic
✦⇄⚒ = designer-to-installer translation logic

Protocol III: Ordinatio

Glyph: ≡
Meaning: information is sorted into the correct region.

Designer info goes to Regio Designii.
Furniture info goes to Regio Mobilia.
Labor info goes to Regio Laboris.
Field info goes to Regio Campi.
Paperwork goes to Regio Documentorum.

Protocol IV: Actio

Glyph: ⚡
Meaning: the system produces the needed action.

Actions include:

generate bid
create estimate
ask for missing info
flag risk
create change order
update punch list
summarize field issue
prepare install packet
create client-ready document
Protocol V: Reductus

Glyph: ↺
Meaning: field reality returns back into the system.

The field is not separate.

The field updates the estimate, the paperwork, the schedule, and the project memory.

5. Platform Intelligence Layers
AI Layer: Intellegentia Operativa

Handles reasoning, classification, summarization, document creation, and estimate logic.

Bot Layer: Nuntii

Small task bots that execute narrow actions:

email bot
PDF bot
estimate bot
quote bot
punch bot
field-note bot
schedule bot
Agent Layer: Auctores

Higher-level agents that understand full workflows.

Sub-Agent Layer: Ministri

Specialized helpers inside each region.

AGI Concept Layer: Mens Operis

Not a claim of true AGI. In this charter, it means the long-term intelligence goal: a system that understands the full life of a furniture/interiors project from design to installation to closeout.

6. Master Compression Glyph Set
Glyph	Name	Meaning
⌬	Nexus	compressed operational intelligence
✦	Designium	designer intent
◈	Mobilia	furniture object intelligence
⚒	Laboris	install labor
⟁	Campus	field condition
▣	Documentum	official paperwork
⇄	Interpres	translation between parties
◉	Memoria	project memory
⚡	Actio	generated action
↺	Reductus	field feedback loop
≡	Ordinatio	structured sorting
◈$	Pretium Mobilia	furniture pricing
⚒$	Pretium Laboris	labor pricing
⟁✓	Veritas Campi	field verification
▣✎	Scriptura	document generation
✦⇄⚒	Design-to-Field	designer intent converted into install instructions
7. One-Sentence Product Definition

Spatium Computationis is an activated computing ecosystem that sits between interior designers, furniture vendors, contractors, and field installers, compressing all project information into estimates, documents, instructions, updates, and execution-ready intelligence.

8. Repo-Level Framing

This should be treated as a module inside a larger repo, not the entire repo.

Current repo structure:

/spatium-computationis
  /agents
    auctor-operis          — Master orchestration agent
    estimator-mobilia      — Furniture pricing and budget agent
    estimator-laboris      — Labor estimating agent
    inspector-campi        — Field condition and punch-list agent
    scriptor-documentorum  — Document generation agent
    interpres-designii     — Designer-to-field translation agent
    custos-memoriae        — Project memory and continuity agent
    adaptio-mentis         — Adaptive learning agent
    adversary-lab          — Adversarial testing agent
    analytics-inspector    — Analytics and reporting agent
    content-creator        — Content creation agent
    defensor-campi         — Field defense agent
    error-eyes             — Error detection and monitoring agent
    gatekeeper-porta       — Access control and gating agent
    marketing-strategist   — Marketing strategy agent
    research-realm         — Research and discovery agent
    seo-optimizer          — SEO optimization agent
    shadow-decryptor       — Shadow traffic analysis agent
    social-media-pilot     — Social media management agent
    vigil-operis           — Operations monitoring agent
    webmaster-godaddy      — Web hosting and domain management agent

  /protocols
    ingressus.py           — Information intake
    compressio.py          — Raw info → compressed intelligence
    ordinatio.py           — Sorting into correct region
    actio.py               — Action generation
    reductus.py            — Field feedback loop
    feedback.py            — Outcome recording and learning signals
    /audit                 — Audit trail protocols
    /collaboration         — Multi-agent collaboration
    /communication         — Inter-system messaging
    /data                  — Data handling protocols
    /discovery             — Service and agent discovery
    /monitoring            — Health and performance monitoring
    /resilience            — Fault tolerance and recovery
    /security              — Security protocols
    /sync                  — Synchronization protocols
    /workflow              — Workflow orchestration

  /defense
    organism_charter.py    — Traffic classification (Cooperative/Hostile/Shadow)
    organism_stats.py      — Defense statistics
    /cloudflare-worker     — Edge defense layer
    /dashboard             — Defense monitoring UI
    /fingerprinting        — Request fingerprinting
    /honeypot              — Threat attraction and analysis
    /threat_intel          — Threat intelligence feeds

  /governance
    hierarchy.py           — 7-level authority hierarchy
    code_of_conduct.py     — Behavioral rules
    rules_of_procedure.py  — Procedural governance
    open_data_policy.py    — Transparency rules
    safety_rules.py        — Safety constraints

  /integrations
    /nova_sovereign        — Nova Sovereign intelligence backend
    godaddy_client.py      — GoDaddy hosting integration
    marketing_store.py     — Marketing data store

  /platform
    registry.py            — Agent and service registry
    delegation.py          — Task delegation engine
    task_queue.py          — Async task processing
    nuntii.py              — Messaging bots

  /scaffolds
    base.py                — Agent lifecycle and capability declarations
    event_bus.py           — Event-driven communication
    orchestrator.py        — Multi-agent orchestration

  /frontend
    routes.py              — Web application routes
    /static                — Static assets
    /templates             — HTML templates

  /glyphs
    glyph-map
    compression-language

  /documents
    bid-generator
    scope-generator
    change-order-generator
    punch-list-generator
    closeout-generator

  /estimating
    furniture-pricing
    labor-pricing
    freight-assumptions
    install-complexity

  /field
    photo-intake
    field-notes
    issue-tracking
    completion-verification
9. Intelligence Backend

The system uses Nova Sovereign (FreddyCreates/Decentralized-Production-NOVA-Protocol) as its intelligence backend.

Required environment variables:
- NOVA_SOVEREIGN_URL — endpoint for the Nova Sovereign service
- NOVA_SOVEREIGN_TOKEN — authentication token

10. Defense System

The Organism Charter classifies all incoming traffic into three tiers:
- Cooperative — routed to Knowledge Realm for processing
- Hostile — routed to Adversary Lab for analysis
- Shadow — routed to Quarantine for isolation and decryption

11. Governance

Authority is enforced through a 7-level hierarchy:
1. Constitutional (highest)
2. Procedural
3. Committee
4. Personnel
5. Ethics
6. Openness
7. Safety (override power)

12. Final North Star

The system's job is not to "make estimates."

The system's job is to understand the entire flow of furniture and interiors work, then turn every piece of information moving between designer, contractor, vendor, installer, and client into the correct action, document, price, instruction, or decision.

⌬ = compressed project intelligence.
